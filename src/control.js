...
/*
Meant to Control any object like a space ship.
If the active camera is fixet at the object, the view is controled.

Keybpard inputs and VR controlers are paired to axis.
Different modes simulate real or arcade behaviour.


BIKE: eigene class "control(bike)" key bis bike.speed* => bike.root.rotate/locallyTranslate => calc setCamera.*
MAZE: src js class "control(user)" key bis user.speed* => user.root.rotate/locallyTranslate => calc setCamera.*
...
export class Control {
    ...
    constructor(controledObject, scene, engine, ix) {
        ...
        // VR-Controler
        this.vrKeyXA = false;
        this.vrJoystick = new BABYLON.Vector3();


        this.positionOffset = new BABYLON.Vector3(); // Object-Center to Camar-Center
        ...
        window.addEventListener("resize", function() {
            engine.resize(); // TODO: scene.engine ???
        });
...
vrHelper.onControllerMeshLoaded.add((webVRController) => {
    ...
    webVRController.onTriggerStateChangedObservable.add((stateObject) => {
        this.vrJoystick.z = stateObject.value;
    });
...
animate(dSec, vrHelper) { // Control
    ...
    var xPos = 0;
    var zPos = 0;
    if (vrHelper.isInVRMode && this.vrKeyXA && this.scene.moveMode > 1) {
    ...
    if (this.scene.moveMode != 3 || !this.vrKeyXA) yRot = 0;
    /**/ var xPos = 0; var yPos = 0; var zPos = 0; var yRot = 0;

    if (this.scene.hmd) this.scene.hmd.out([
    ...
    var accelInputX = setDeadZone(this.vrJoystick.x) * +1; // +/- 0.2 to 1.0  becomes  0.0 to 1.0
    var accelInputY = setDeadZone(this.vrJoystick.y) * -1;
    var accelInputZ = setDeadZone(this.vrJoystick.z) * +1;
